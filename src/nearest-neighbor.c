...
int *extraiVizinhoMaisProximo(int tamanho,
                              double **vetorResultadoValoresDistancia,
                              float *vetorVizinhoMaisProximo, int debug) {
  int i, j;

  // Vetor que armazenas os índices do vizinho mais próximo
  int *vetorIndicesDistancia = malloc(tamanho * sizeof(int *));

#pragma omp parallel private(i, j)                                             \
    shared(vetorResultadoValoresDistancia, vetorVizinhoMaisProximo,            \
           vetorIndicesDistancia)
  {
#pragma omp for
